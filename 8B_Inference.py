...
MODE = 3
IMAGEDIR = "./testset"
SINGLE_IMAGE_PATH = "./testset/0008.jpg"
...
# ---------------- LOAD CENTROID ----------------
with open(os.path.join(MODEL_DIR, "centroid_labels.pkl"), "rb") as f: # Load centroid labels
    label_names = pickle.load(f)

centers = np.load(os.path.join(MODEL_DIR, "centroid_centers.npy")) # Load centroid embeddings
centers = centers / np.linalg.norm(centers, axis=1, keepdims=True)
models["centroid"] = centers # Save normalised centroids
...
# -------------------------------------------- CENTROIDS ----------------
                if model_name == "centroid":
                    sims = models["centroid"] @ emb # Cosine similarity = dot product -> sims = list of cosine similarities
                    idx = int(np.argmax(sims)) # Return index of highest cosine similarity
...
if MODE == 3:
    ...
    if img is None:
        raise ValueError(f"Image not found: {SINGLE_IMAGE_PATH}")
